...
class mparametro{
	...
	function selpar1($codpar){
		...
		$sql = "SELECT parametro.codpar ,parametro.nompar ,valor.codval, valor.nomval  FROM parametro LEFT JOIN valor ON parametro.codpar=valor.codpar ";
		if($filtro)
			$sql.= " WHERE parametro.nompar LIKE '%".$filtro."%'";
		$sql .= "  ORDER BY parametro.codpar LIMIT ".$rvalini.", ".$rvalfin;
